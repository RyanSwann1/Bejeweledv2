	...
	for (auto &bindIter : m_bindings)
	{
		...
		for (auto &eventIter : bind.m_events)
		{
			...
			case (EventType::KeyPressed):
			{
				...
				{
					activateEvent(bind, m_sharedContext->m_stateManager->getCurrentState());
				}
				break;
			}
...
{
	std::ifstream file;
	file.open(Utilities::getResourceLocation() + "Keys.txt");
	if (!file.is_open())
	{
		std::cerr << "Failed to open: " << Utilities::getResourceLocation() + "Keys.txt" << "\n";
	}
	std::string line;
